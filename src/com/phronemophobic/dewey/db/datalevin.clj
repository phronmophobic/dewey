(ns com.phronemophobic.dewey.db.datalevin
...
(defn index->db [release-id]
  (let [full-analysis (analyses-iter
                       (io/file "releases"
                                release-id
                                "analysis.edn.gz"))
        ]
    (doseq [[i a] (map-indexed vector full-analysis)
      ...
      (let [tx (analysis->tx release-id
                             a)
            ;; create a new connection per transaction with
            ;; with hopes that it helps with memory usage
            conn (d/get-conn "db/" schema)]
        (try
...
(comment
  (time (index->db "2024-09-30"))


  (def full-analysis (analyses-iter "releases/2024-09-30/analysis.edn"))
  (def analysis
